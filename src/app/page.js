...
import NeuralBackground from "@/components/NeuralBackground"; // <--- NEW IMPORT
import LiquidText from "@/components/LiquidText"; // <--- NEW IMPORT
import { useLang } from "@/context/LanguageContext";

export default function Home() {
  const { t, lang } = useLang();
  
  const typeWords = lang === 'si' ? ["වෙබ් අඩවි", "සොෆ්ට්වෙයාර්", "බ්‍රෑන්ඩ්ස්"] : ["WEBSITES", "SOFTWARE", "BRANDS"];
    ...
    <div className="flex flex-col w-full overflow-hidden relative">
      ...
      <div className="relative z-10 bg-black/20 backdrop-blur-sm">
        ...
        <div className="px-6 max-w-7xl mx-auto">
           <ParallaxImage src="https://images.unsplash.com/photo-1550751827-4bd374c3f58b?q=80&w=1200" alt="Cyber Engineering" />
        </div>
        <ComparisonSlider />
        <SiteAudit /> 
        ...
        <section className="py-24 px-6 bg-surface/50">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              {/* Other service cards omitted for brevity, but they are here */}
              <FadeIn delay={0.2}><div className="p-10 bg-background rounded-3xl border border-gray-800 hover:border-accent transition-colors h-full flex flex-col items-center text-center shadow-2xl group"><div className="w-16 h-16 bg-accent/10 rounded-2xl flex items-center justify-center text-3xl mb-6">⚙️</div><h3 className="text-2xl font-bold text-white mb-4 font-display">{t('services.soft.title')}</h3><p className="text-gray-400 leading-relaxed">{t('services.soft.desc')}</p></div></FadeIn>
              <FadeIn delay={0.3}><div className="p-10 bg-background rounded-3xl border border-gray-800 hover:border-purple-500 transition-colors h-full flex flex-col items-center text-center shadow-2xl group"><div className="w-16 h-16 bg-purple-500/10 rounded-2xl flex items-center justify-center text-3xl mb-6">🎨</div><h3 className="text-2xl font-bold text-white mb-4 font-display">{t('services.brand.title')}</h3><p className="text-gray-400 leading-relaxed">{t('services.brand.desc')}</p></div></FadeIn>
            </div>
          </div>
        </section>
