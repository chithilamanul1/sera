...
import BrandIcon from "./BrandIcon"; // <--- NEW ICON
...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-500 ${scrolled ? 'bg-background/80 backdrop-blur-xl border-b border-white/5 py-3' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        
        {/* LOGO AREA (Icon + Text) */}
        <Link href="/" className="flex items-center gap-3 group">
           <BrandIcon className="w-10 h-10 group-hover:scale-110 transition-transform duration-300" />
      ...
      {/* MOBILE MENU */}
      {mobileOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full h-screen bg-black p-10 flex flex-col gap-8 z-40 border-t border-gray-800">
